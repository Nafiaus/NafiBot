...
#define OAUTH "your twitch oauth"
...
// Declare servos/position, leds, cooldown, and irc channel
bool laserOn = false;
int timer = 1000;
int xpos = X_DEFAULT;
int ypos = Y_DEFAULT;
...
void laserMode() {
  if(laserOn == false) {
    ...
    sendTwitchMessage("Uhm, dumbass... Check your dogshit, thrown together, rinky dink, \"I\'m a programmer\" ass arduino code. ERROR with bool laserOn in laserMode()");
  }
}

// Laser Command cause I hate myself
void shootLaser() {
  if(laserOn == false) {
    digitalWrite(L_PIN, HIGH);
    delay(5000);
    ...
    sendTwitchMessage("Cannot shoot a laser that's already FUCKING ON... :) <3");
  } else {
    sendTwitchMessage("Uhm, dumbass... Check your dogshit, thrown together, rinky dink, \"I\'m a programmer\" ass arduino code. ERROR with bool laserOn in shootLaser()");
  }
}
